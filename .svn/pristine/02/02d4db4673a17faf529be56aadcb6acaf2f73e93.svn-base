                    ...
                    if (element.sysCode == 'eRecord') {
                      ...
                      if (flag) {
                        ...
                        async function fechUseObj() {
                          ...
                          console.log(11,usrObj)
                          if (usrObj && usrObj.orgCode && usrObj.orgCode != getUserMsgdata.orgCode) {
                            console.log("上级机构跳转时不重新保存用户信息main.html");
                          } else {
                            _this.ObjArray = getUserMsgdata.data;
                            if(_this.ObjArray.userAuthList.length>1){
                              _this.ObjArray.userAuthList.splice(1,_this.ObjArray.userAuthList.length)
                            }
                            api.setGlobalVal('userObj', JSON.stringify( _this.ObjArray));
                            // console.log(JSON.stringify(getUserMsgdata.data));
                            // console.log(api.getGlobalVal('userObj'))
                            localStorage.setItem("userId",api.getGlobalVal("userObj").id);
                            localStorage.setItem("userName",api.getGlobalVal("userObj").userName);

<template>
  <div>
      ...
      <div class="newTable">
        ...
        >
          ...
          <el-table-column align="center" min-width="10%" prop="oprFlag" label="任务类型" sortable>
            <template slot-scope="scope">
              {{showLabel(scope.row.oprFlag,oprFlagList)}}
              {{scope.row.oprFlag}}
            </template>
          </el-table-column>
          <el-table-column align="left" min-width="10%" prop="recordStatus" label="任务状态" sortable>
            <template slot-scope="scope">
              {{showLabel(scope.row.recordStatus,recordStatusList)}}
    ...
    <!--    新增弹出框   -->
    <el-dialog title="新增" :visible.sync="dialogAddVisible" width="520px">
      <div class="height-300">
        <Upload :uploadDataVal="uploadDataVal" v-on:saveHandel="saveHandelNewAdd"></Upload>
      </div>
    </el-dialog>
...
export default {
  ...
  mounted() {
    const that = this;
    that.roleName = api.getGlobalVal(
      "userObj"
    ).userAuthList[0].authRoleList[0].roleName;
    //视口缩放时候 maxheight跟着改变
    if (that.$refs.table) {
      let maxheight = that.$refs.table.$el.offsetTop;
      that.$nextTick(function() {
        that.tableHeight = window.innerHeight - maxheight - 130;
        // 监听窗口大小变化
  ...
  methods: {
      ...
      api
        .post(
          "/remoteApi/tool/org/findOrgListByRole",
          {
            ...
            roleName: api.getGlobalVal("userObj").userAuthList[0]
              .authRoleList[0].roleName
          },
          5000
        )
        .then(res => {
      ...
      api
        .post(
          "/remoteApi/tool/recordTrial/findRecordTrialList",
          {
            ...
            roleNames: api.getGlobalVal("userObj").userAuthList[0]
              .authRoleList[0].roleName,
            roleIds: api.getGlobalVal("userObj").userAuthList[0].authRoleList[0]
              .roleId
          },
          5000
        )
    ...
    handleDelete(index, row) {
      this.$confirm("确定要删除吗?", "提示", {
        ...
        .then(() => {
          api
            .post(
              "/remoteApi/tool/recordTrial/deleteRecordTrialById",
              {
                userId: api.getGlobalVal("userObj").id,
    ...
    handleEdit(index, row) {
      ...
      this.editUploadData = {
        editVal: {
          fileUrl: row.fileUrls != "" ? row.fileUrls.split(",") : null,
          fileName: row.fileName != "" ? row.fileName.split(",") : null,
          oprFlag: row.oprFlag,
    ...
    handleDown(index, row) {
      this.$confirm("确定要下载吗?", "下载提示", {
        ...
        .then(() => {
          window.open("/remoteApi/tool/record/downAttachment?fileId=" + row.id);
        })
        .catch(err => {
          this.$message({
            type: "warning",

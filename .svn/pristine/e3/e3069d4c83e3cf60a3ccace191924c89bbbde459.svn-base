<template>
  <div>
    <!-- 详情页 start-->
    <el-dialog title="设施巡查任务详情" :before-close="handleClose" :close-on-click-modal="false" :visible.sync="showDetail" width="916px" style="margin: 0 auto; text-align: left;">
      <el-scrollbar>
            ...
            <table class="gg-table2 gg-tableFix" cellspacing="0" cellpadding="0" v-if="problemList.length > 0" style="border-bottom: 1px solid #e9e9e8;">
              <tbody>
                ...
                <div v-for="(item, index) in problemList" :key="index" class="black borders" style="width: 1020px; padding: 20px;">
                  <h2 style="font-size: 18px;">{{item.itemName}}</h2>

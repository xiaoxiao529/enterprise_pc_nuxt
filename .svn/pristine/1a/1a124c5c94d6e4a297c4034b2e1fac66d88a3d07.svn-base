  ...
  export default {
    ...
    data() {
      return {
        ...
        fullInfo: {
          pageName: "维保合同",
          tit: '新增维保合同',
          addUrl: api.forent_url.special_base_url + "/maintContract/uploadFile",
          downloadUrl: api.forent_url.base_url + "/maintContract/download?id=",
          deleteUrl: api.forent_url.special_base_url + "/maintContract/del"
        },
        widgetInfo: {
          tHeaders: [{
            prop: "contractName",
    ...
    methods: {
      ...
      getData(safetyAttachName) {
        this.loading = true;
        api.post(api.forent_url.base_url + "/maintContract/query", {
          orgId: this.userObj.orgCode,
          attachmentTypeCode: "WBHT",
          pageSize: this.widgetInfo.pageSize,
          pageNo: this.widgetInfo.pageNo,
          contractName: safetyAttachName
        }).then(res => {
          if (res.message == "ok") {

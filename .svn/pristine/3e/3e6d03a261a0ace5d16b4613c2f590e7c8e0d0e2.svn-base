<template>
  <div class="main">
    <div :class='["grid-content", "gg-ditu",roleClassName]'>      
      <!-- 地图组件 -->
      <G2Map v-on:changeShowType="changeShowType" v-on:changeBigSmall="changeBigSmall" :bigScreen="bigScreen"></G2Map>
  ...
  export default {
    ...
    methods: {
      ...
      alarmBox(num) {
        ...
        api.post(api.forent_url.base_url + '/warning/queryLyMonitorInfoList', {
            proprietorId: this.userObj.orgCode,
            runState: this.statusNum
          })
          .then(res => {
            // console.log(res);
            if (res.code == "success") {
      ...
      changeM: function (url, lightNav, parName, pathName) {
        if (pathName == '隐患统计') {

          if (api.getGlobalVal("loginName") == 'qujl') {
            lightNav = '5-2'
          } else {
            lightNav = '5-1'
          }
      ...
      //临沂报警数量信息
      linyiQueryMonitor: function () {
        var that = this;
        api.post(api.forent_url.base_url + '/warning/queryLyMonitorInfo?proprietorId=' + that.userObj.orgCode).then(
          res => {
            if (res.code == "success") {
              var data = res.data;
              if (data != "" && data != null) {
                that.alarmCount_linyi = data.alarmCount
                that.warningCount_linyi = data.warningCount
                that.troubleCount_linyi = data.troubleCount
              }
            }
          })
      },
      showTaskDetail: function (row) {
        this.showDetail[row.inspectionType] = true;
        this.rowObj[row.inspectionType] = row;
        // console.log(a, b, c, d)
      },
      showBigScreen: function () {
        ...
        if (this.bigFlag) {
          ...
          // this.ZS.MONITOR.setViewSize($('#g2map').width(), $('#g2map').height());
      ...
      raceLamp() {
        var index = -1;
        var leth = $("li.float li").length / 2;
        setInterval(function () {
          index++;
          if (index > leth) {
            index = -1;
            $("li.float li").removeClass('current');
            return false;
          }
          $("li.float li").eq(index).addClass('current');
          $("li.float li").eq(index + leth).addClass('current');
        }, 100)
      }
    },
    mounted() {

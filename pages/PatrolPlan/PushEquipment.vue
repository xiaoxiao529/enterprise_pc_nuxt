<template>
    <div class="storied-suilding-wrapper right-main-warpper">
    	<div class="h2-title-wrap">
	        <h2 class="tilte-h2">推送缓存</h2>
	      </div>
        <div class="search-bar-wrapper">
            <el-form label-position="left" inline class="demo-table-expand">
                ...
                <el-form-item label="回路:" :label-width="searchFormLabelWidth" style="width: 18.75%">
                    <el-input  v-model="formInline.laLoop"></el-input>
                </el-form-item>
                <el-form-item label="点位:" :label-width="searchFormLabelWidth"  style="width: 18.75%">
                    <el-input  v-model="formInline.laPoint"></el-input>
                </el-form-item>
                <el-form-item label="用传编码:" :label-width="searchFormLabelWidth"  style="width: 18.75%">
                    <el-input  v-model="formInline.controlHost"></el-input>
                </el-form-item>
                <el-form-item label="状态:" :label-width="searchFormLabelWidth" style="width: 18.75%">
                ...
                <el-form-item  class="t-right">
                    ...
                    <el-button type="primary" @click="pushCache()">推送缓存</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="notab">
            <el-scrollbar>
                <el-table v-loading="isLoading" :data="tableData" stripe>
                    <el-table-column align="left" min-width="7%" :show-overflow-tooltip="true" pageSize="pageSize" prop="deviceId"
                                     label="设备Id">
                    </el-table-column>
                    <el-table-column align="left" min-width="10%" :show-overflow-tooltip="true" prop="deviceName" label="设备名称">
                    </el-table-column>
                    <el-table-column align="left" min-width="8%" :show-overflow-tooltip="true" prop="deviceTypeName" label="设备类型">
                    </el-table-column>
                    <el-table-column align="left" min-width="8%" :show-overflow-tooltip="true" prop="deviceAddress" label="设备地址">
                    </el-table-column>
                    <el-table-column align="left" min-width="8%" :show-overflow-tooltip="true" prop="laLoop" label="机号回路点位">
                        <template slot-scope="scope">
                            {{scope.row.laMake}}-{{scope.row.laLoop}}-{{scope.row.laPoint}}
                        </template>
                    </el-table-column>
                    <el-table-column align="left" min-width="7%" :show-overflow-tooltip="true" prop="twoCode" label="二次码">
                    </el-table-column>
                    <el-table-column align="left" min-width="7%" :show-overflow-tooltip="true" prop="sourceId" label="sourceId">
                    </el-table-column>
                    <el-table-column align="left" min-width="8%" :show-overflow-tooltip="true" prop="controlHost" label="controlHost">
                    </el-table-column>
                    ...
                    <el-table-column align="center" min-width="12%" label="操作">
                        <template slot-scope="scope">
                            ...
                            <el-button type="text" @click="handleEditPlan('delete',scope.row)"  v-if="facilitiesInData.limtShow" :disabled="scope.row.status=='0' ? false : true"
                            >停用</el-button>
                        </template>
                    </el-table-column>
                </el-table>
      ...
      required: true, message: '楼栋不能为空', trigger: 'blur'}">
                        <el-select :disabled="inputDisabled" v-model.trim="plan.buildId" filterable placeholder="楼栋信息" @change="getFloorInfo" >
                            <el-option v-for="item in buildList" :key="item.buildId" :label="item.buildName" :value="item.buildId">
                            </el-option>
                        </el-select>
                    </el-form-item>
      ...
      required: true, message: '楼层不能为空', trigger: 'blur'}">
                        <el-select :disabled="inputDisabled" v-model.trim="plan.floorId" filterable placeholder="楼层信息" @change="getFloorInfoName" >
                            <el-option v-for="item in floorList" :key="item.floorId" :label="item.floorName" :value="item.floorId">
                            </el-option>
                        </el-select>
                    </el-form-item>
      ...
      required: true, message: '设备名称不能为空', trigger: 'blur'}">
                    ...
                    <el-form-item v-if="optMode != 'add'" label="设备ID:" :label-width="formLabelWidth">
                        <el-input disabled v-model="plan.deviceId" auto-complete="off"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="是否为C端设备:" :label-width="formLabelWidth">
                        <el-select :disabled="inputDisabled" v-model.trim="plan.isCTerminal" placeholder="是否为C端设备">
                            <el-option v-for="item in terminalList" :key="item.value" :label="item.label" :value="item.value">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="百度坐标X:" :label-width="formLabelWidth" >
                        <el-input :disabled="inputDisabled" v-model="plan.mapX" auto-complete="off"  placeholder="请输入数字"></el-input>
                    </el-form-item>
                    <el-form-item label="百度坐标Y:" :label-width="formLabelWidth" >
                        <el-input :disabled="inputDisabled" v-model="plan.mapY" auto-complete="off"  placeholder="请输入数字"></el-input>
                    </el-form-item>
        ...
        <el-dialog
		  ...
		  width="550px"
		>
		  <div class="CacheBtn" style="padding: 30px 0;">
  			<el-radio-group v-model="formInline.pushCacheValue" @change="changeCacheValue">
              <el-radio  v-for="item of pushCacheData" :key="item.value" :label="item.value">{{item.label}}</el-radio>
            </el-radio-group>
		  </div>
		  
		  <span slot="footer" class="dialog-footer">
		    <el-button @click="cencelPushCache()">取 消</el-button>
		    <el-button type="primary" @click="openDoubleDialog()">确 定</el-button>
		  </span>
		</el-dialog>
		...
		<el-dialog
		  ...
		  width="550px"
		>
		  <p style="text-align: center;font-size: 16px;padding: 30px 0;">确定要推送缓存吗？</p>
		  
		  <span slot="footer" class="dialog-footer">
		    <el-button @click="cencelPushCache()">取 消</el-button>
		    <el-button type="primary" @click="pushCacheSure()">确 定</el-button>
		  </span>
		</el-dialog>
...
//      props:['facilitiesInData'],
        data(){
            return{
                ...
                facilitiesInData:{},
                pushCacheData:[
                	{value:'NewDevices',label:"设备新增"},
                    {value:'UpdateDevices',label:"设备更新"},
                    {value:'LogicalDelDevices',label:"逻辑删除"},
                    {value:'PhysicsDelDevices',label:"物理删除"}
                ],
...
//      	获取缓存值
			changeCacheValue(event){
				console.log(this.formInline.pushCacheValue)
			},
			openDoubleDialog(){
				if(this.formInline.pushCacheValue == ""){
					this.$message({
                        type: 'warning',
                        message: "请选择推送类型!"
                   });
			...
			cencelPushCache(){
				this.dialogVisible = false;
				this.dialogDoubleVisible = false;
				this.formInline.pushCacheValue = "";
			},
			pushCacheSure(){
				...
				this.dialogDoubleVisible = false;
				api
		        .post("/remoteApi/tool/deviceInfo/pushCache", {
		          	unitId: this.unitId,
		          	pushType: this.formInline.pushCacheValue,
...
//                  unitId: this.facilitiesInData.unitId || "",
            ...
            viewHandle(planObj) {
                ...
                api
                    .get("/remoteApi/tool/deviceInfo/getDeviceById", {
                        id: planObj.id
                    })
                    .then(res => {
            ...
            saveOrUpdateForm(){//保存或更新操作
                ...
                if(that.plan.mapX && !reg.test(that.plan.mapX)){
                    that.$message({
                        message: "百度坐标X为数字",
                        type: "warning"
                    });
                ...
                if(that.plan.mapY && !reg.test(that.plan.mapY)){
                    that.$message({
                        message: "百度坐标Y为数字",
                        type: "warning"
                    });

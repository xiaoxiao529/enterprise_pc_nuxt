...
export default {
  ...
  methods: {
      ...
      api
        ...
        .then(res => {
          if (res.code === "success") {
            this.loading = false;
            if (res.data && res.data.length > 0) {
              that.allUnitList = [{label:'全部',value:'',subCenterCode:''}];
              res.data.map(function(item) {
    ...
    submitAddBtn() {
      ...
      that.uploadDataVal.timestamp = new Date().getTime();
      console.log(that.uploadDataVal.timestamp)
    },
    //新增弹框关闭
    handleClose(){
      var that = this;
      that.dialogAddVisible = false;
      that.$refs.myChild.close()
